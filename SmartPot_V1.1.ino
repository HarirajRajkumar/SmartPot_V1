 ...
 *     CONTROL :
 *     Turns on Red LED and Yellow LED 
 *     indicating Temperature and humidity is high in that kept environment.
 *     You can feel free to change the threshold of the temperature and humidity in the Threshold Area
 *     
 *     
 ...
 *     
 *     If the moisture content is low in the soil ie 1023 analog read value ( compares with the threshold value ie Moisture_Thres )
 *     it takes water from the reservoir only when there is water present 
 *     above the low level and moisture is below the threshold 
...
// No water blink indication constents
long LEDblinkPrevious;
...
{ 
 ...
 // Uncomment to read the moisture level
...
// Note : You can change the threshold value.. It's all yours
 ...
 {    
 if ( MilliTimer - previousTimer > 25000 )  // Air Pump ON for 10 seconds through relay 
  // Moisture
  {
  ...
  float V_Mid = (analogRead ( A2 )*5.12)/1024;    // Reads the second stage ( Middle ) of the reservoir
 ...
 if ( V_Mid >= 1 )
  ...
  float V_High = (analogRead (A3 )*5.12)/1024;    // Reads the third stage of the reservoir ( High )
